...
impl FontImpl {
    pub fn new(
        ...
        let glyph_gamma = 1.0; //if pixels_per_point == 1.0 { 1.4 } else { 1.1 };
    ...
    fn glyph_info(&self, c: char) -> Option<GlyphInfo> {
        ...
        let glyph_shift = match (c, self.name.as_str(), self.font_size) {
            ...
            ('+', "JetBrainsMono", 11.0) => (0.0, -0.4),
            ('1', "JetBrainsMono", 11.0) => (-0.4, -0.0),
            ('0', "JetBrainsMono", 11.0) => (-0.05, -0.0),
            ...
            // Phosphor icons
            (/* BOOK */ '\u{E0E2}', "phosphor", 16.0) => (0.5, 0.5),
            (/* PACKAGE */ '\u{E390}', "phosphor", 16.0) => (-0.5, -0.0),
